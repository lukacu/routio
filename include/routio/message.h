...
namespace routio
{
    ...
    public:
        ...
        virtual void inspect_data(ostream& output) const;
    };
    ...
    public:
        ...
        template <class T>
        static shared_ptr<Message> pack(const T &data);
        template <typename T>
        static shared_ptr<T> unpack(SharedMessage message);
    ...
    public:
        ...
        template <class I>
        MultiBufferMessage(I begin, I end) : buffers(begin, end) { rebuild(); }
    ...
    template <>
    inline string MessageReader::read<string>()
    {
        return read_string();
    }
    ...
    template <typename T>
    void read(MessageReader &reader, T &dst)
    {
        static_assert(std::is_arithmetic<T>::value, "Only primitive numeric types supported");
        dst = reader.read<T>();
    }
    ...
    template <>
    inline void read(MessageReader &reader, string &dst)
    {
        dst = reader.read_string();
    }

    template <typename T>
    void read(MessageReader &reader, vector<T> &dst)
    {
    ...
    template <>
    inline void MessageWriter::write<string>(const string &value)
    {
        write_string(value);
    }

    template <>
    inline void MessageWriter::write<bool>(const bool &value)
    {
        write_bool(value);
    }

    template <typename T>
    void write(MessageWriter &writer, const T &src)
    {
        static_assert(std::is_arithmetic<T>::value, "Only primitive numeric types supported here");
        writer.write<T>(src);
    }
    ...
    template <>
    inline void write(MessageWriter &writer, const string &src)
    {
        writer.write_string(src);
    }

    template <typename T>
    void write(MessageWriter &writer, const vector<T> &src)
    {
    ...
    template <typename T>
    size_t message_length(const T data)
    {
    ...
    public:
        ...
        template <class T>
        T get(const string &key, const T &value) const;
...
#define MESSAGE_CALLBACK_DROPPED 1
    ...
    protected:
        ...
        public:
            ...
            MessageContainer(SharedMessage message, int priority, long time, MessageCallback callback = NULL) : message(message), priority(priority), time(time), callback(callback) {}
