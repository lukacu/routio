...
* Routio is an interprocess communication library. It allows different processes or programs to share data between themselves.
* Routio uses a publish/subscribe system to allow for organized sharing of data between processes. All communication is organized through named channels. 
...
* Routio uses a central process (also called a dameon) to manage it's communication. This the deamon needs to run in the background if you want to use the library.
  To start the router, navigate to the build directory of the project and run ./routio-router socket_path, where socket path is the path to a socket through which the processes
  will communicate. The programs you are writing will use this socket path to connect with the router.
* All Routio methods are thread safe.
* Routio natively uses recent C++11 and C++14 features such as smart pointers and anonymous functions. This means that you don't need to use libraries like boost for this functionallity.


Writing a simple program
------------------------
Let's examine the included chat example to get an idea of how to write programs using Routio. A chat application is one of the simplest examples of interprocess communication.::

    #include "client.h"
    #include "datatypes.h"

    template<> inline shared_ptr<Message>
    routio::Message::pack<pair<string, string> >
    ...
    routio::Message::unpack<pair<string, string> >(SharedMessage message)
    {
...
First, we need to define the datatype we will be sending. In this example, we will be sending a pair of two strings - a pair<string,string>. If we want to transfer messages between two processes, we first need to tell routio how to convert them into an appropriate binary form. To do this, we need to define two functions, one to encode the object into a binary representation and one to decode the received object from it's binary representation. This is done through the first two function::

    template<> inline shared_ptr<Message>
    routio::Message::pack<pair<string, string> >(int channel, const pair<string, string> &data)
    {
        MessageWriter writer(data.first.size() + data.second.size() + 8);
