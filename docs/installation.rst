...
Echo is based on a central process or router, which runs in the background and manages the communication between processes. Because of this, the router process must always be running in the background when you wish to use Echo. When starting the router process, you must prowide a socket name that other proccesses will connect to. To start the daemon, simply run::
